<?php
class module_mods_normal_viewTable extends abstract_moduleBuilder{

	protected $sModule='mods_normal_viewTable';
	protected $sModuleView='mods/normal/viewTable';
	...
	private function processSimple(){
		...
		if(!file_exists(_root::getConfigVar('path.generation')._root::getParam('id').'/module/table')){
		...
		$oFile=new _file(_root::getConfigVar('path.generation')._root::getParam('id').'/module/'.$sModule.'/view/'.$sView.'.php');
		if($oFile->exist()){
