...
class business_VARmoduleRightsManagerENDVAR extends business_abstract
{
    ...
    {
        $sGroupText=trim($tParam_['VARpermission_fk_group_idENDVAR_text']);
        $sActionText=trim($tParam_['VARpermission_fk_action_idENDVAR_text']);
        $sItemText=trim($tParam_['VARpermission_fk_item_idENDVAR_text']);
        ...
        if ($sActionText!='') {
            $oAction=$this->getActionByName($sActionText);
    ...
    public function insertPermission($oPermission_, $tParam_)
    {
        ...
        foreach ($this->_tColumn as $sColummn) {
            $oPermission_->$sColummn = $tParam[$sColummn];
        }

        $this->_oModel->insert($oPermission_);
    ...
    {
        $this->_oModel->deletePermissionById($id_);
