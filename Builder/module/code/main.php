...
class module_code extends abstract_module{
	...
	public function _editcode(){
		...
		$oModuleDir=new _dir(_root::getConfigVar('path.generation')._root::getParam('project').'/module');
		$tModule=array();
		foreach($oModuleDir->getListDir() as $oDir){
			$tModule[]=$oDir->getName();
		}

		$oModelDir=new _dir(_root::getConfigVar('path.generation')._root::getParam('project').'/model');
		$tModel=array();
